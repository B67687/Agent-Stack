...
Checks performed:
  1. opencode.jsonc exists and ~/.omo/omo.jsonc exists (OMO unified config)
  2. Config files are valid JSON (JSONC comments stripped via sed before jq)
...
# ── Check 3: JSONC validation via jsonc-parser ──
echo ""
echo "--- JSONC Validation (omo.jsonc) ---"
if command -v node &>/dev/null; then
  filepath="$HOME/.omo/omo.jsonc"
  if [ ! -f "$filepath" ]; then
    fail "omo.jsonc: file not found"
  else
    result=$(node scripts/validate-jsonc.js check "$filepath" 2>&1)
    if [ -z "$result" ]; then
      pass "omo.jsonc: valid JSONC"
    else
      fail "omo.jsonc: JSONC parse error"
      echo "       $result"
    fi
  fi
else
  warn "node not found — skipping JSONC validation"
  warn "Install node to enable JSONC validation"
fi

# ── Check 3.5: Config schema validation (installed schema ground truth) ──
echo ""
...
if command -v node &>/dev/null; then
  ...
  if [ "$schema_exit" -eq 0 ]; then
    pass "omo.jsonc: all keys valid against installed schema"
  else
    fail "omo.jsonc: schema violations found"
    echo "       $schema_result" | sed 's/^/       /'
  fi
else
  warn "node not found — skipping config schema validation"
fi

# ── Check 3.6: TUI validation path (staged boot + ghost regression) ──
# Gated behind RUN_SLOW_TESTS=1: boots opencode in a staging HOME (90s worst
# case). Run explicitly or in CI/pre-push; default pre-commit skips it.
echo ""
echo "--- TUI Validation Path Test (tui-validation-path-test) ---"
if [ "${RUN_SLOW_TESTS:-0}" = "1" ]; then
  if [ -f scripts/tui-validation-path-test.sh ]; then
    tui_results=$(bash scripts/tui-validation-path-test.sh 2>&1)
    tui_exit=$?
    if [ "$tui_exit" -eq 0 ]; then
      pass "TUI validation path: staged boot + ghost regression CLEAN"
    else
      fail "TUI validation path: test failed (exit $tui_exit)"
      echo "       $tui_results" | sed 's/^/       /' | tail -20
    fi
  else
    warn "tui-validation-path-test.sh not found — skipping"
  fi
else
  warn "TUI validation path skipped (slow test). Re-run with RUN_SLOW_TESTS=1 to enable."
fi

# ── Check 4: No uncommitted *.pem or .env files ──
