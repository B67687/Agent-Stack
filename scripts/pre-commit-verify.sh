...
echo ""
echo "--- JSONC Validation (oh-my-openagent.jsonc) ---"
if command -v node &>/dev/null; then
  for f in oh-my-openagent.jsonc; do
    filepath="$CONFIG_DIR/$f"
    if [ ! -f "$filepath" ]; then
    ...
    result=$(node -e "
const { parse, ParseErrorCode } = require('$OMO_JSONC_PARSER');
const fs = require('fs');
const content = fs.readFileSync('$filepath', 'utf8');
const errors = [];
parse(content, errors, { allowTrailingComma: true, disallowComments: false });
if (errors.length > 0) {
  errors.forEach(e => console.log('Code ' + e.error + ' (' + (ParseErrorCode[e.error] || '?') + ') at offset ' + e.offset));
}
" 2>&1)
    if [ -z "$result" ]; then
      pass "$f: valid JSONC"
  ...
  # Also validate lsp.json (plain JSON)
  if [ -f "$CONFIG_DIR/lsp.json" ]; then
    if node scripts/validate-jsonc.js check "$CONFIG_DIR/lsp.json" 2>/dev/null; then
      pass "lsp.json: valid JSONC"
    else
      node scripts/validate-jsonc.js check "$CONFIG_DIR/lsp.json" 2>&1
      fail "lsp.json: invalid JSONC"
    fi
  fi
else
