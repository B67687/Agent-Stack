...
VERSIONS_FILE="$PROJECT_DIR/README.md"
...
main() {
  ...
  omo_cur=$(tracked_version "oh-my-openagent")
  ...
  oc_latest_json=$(get_latest_gh_release "anomalyco/opencode")
  ...
  if [ -z "$oc_latest_tag" ]; then
    red "  FAILED — GitHub CLI unreachable for anomalyco/opencode" 1>&2
  elif [ "$oc_latest_tag" != "$oc_cur" ]; then
    if release_aged_check "$oc_latest_date"; then
      green "  UPDATE: $oc_cur → $oc_latest_tag" 1>&2
      local oc_notes
      oc_notes=$(get_release_notes "anomalyco/opencode" "$oc_latest_tag")
      updates=$(echo "$updates" | jq \
        --arg pkg "OpenCode" \
        --arg cur "$oc_cur" \
        ...
        '. + [{
          ...
          "latest_version": $latest,
          "notes": $notes,
          "source": "github-releases (anomalyco/opencode)",
