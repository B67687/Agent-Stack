...
import json5, re
models = set()
# OMO unified config: top-level 'model' key (4.19.4 runtime shape) or models[] arrays, under the '[opencode]' scope
omo = json5.loads(open('$OMO').read())['[opencode]']
for scope in ['agents', 'categories']:
    for name, obj in omo.get(scope, {}).items():
        ...
        for entry in obj.get('models', []):
            ...
            if m:
                models.add(m)
# OpenCode base config: scan raw text for model keys
...
if [ -n "$NEW_PREMIUM" ]; then
    ...
    echo ""
    echo "  Note: These are blocked by disabled_providers + the fallback_models allowlist."
