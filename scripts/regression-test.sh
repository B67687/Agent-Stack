...
for f in opencode.jsonc dcp.jsonc omo.jsonc; do
    if [ "$f" = "omo.jsonc" ]; then
...
# ── Test 5: fallback_models allowlist enforcement ──
echo ""
echo "--- Fallback Allowlist ---"
# Multi-entry fallback_models is CORRECT design (2026-08-03, suppresses the
# hardcoded AGENT_MODEL_REQUIREMENTS claude-opus-5 chain). Enforce instead that
# every fallback entry is in the allowlist and no forbidden provider appears.
FB_CHECK=$(python3 -c "
import json5
omo = json5.loads(open('$HOME/.omo/omo.jsonc').read())
omo = omo['[opencode]']
allowed = {'opencode-go/deepseek-v4-flash', 'opencode-go/gpt-5.6-luna', 'opencode-go/minimax-m3', 'opencode-go/mimo-v2.5', 'opencode-go/glm-5.2', 'opencode-go/muse-spark-1.2-contributor', 'opencode-go/muse-spark-1.3-contributor', 'opencode/mimo-v2.5-free'}
forbidden_providers = ('anthropic', 'openai', 'google', 'xai')
bad = []
for scope_name in ['agents', 'categories']:
    for name, obj in omo.get(scope_name, {}).items():
        fbs = obj.get('fallback_models') or []
        for fb in fbs:
            if fb not in allowed:
                bad.append(f'{scope_name}/{name}: disallowed fallback {fb}')
            if any(fp in fb for fp in forbidden_providers):
                bad.append(f'{scope_name}/{name}: forbidden provider in fallback {fb}')
if bad:
    print('; '.join(bad))
else:
    print('ok')
")
if [ "$FB_CHECK" = "ok" ]; then
    pass "All fallback_models entries in allowlist; no forbidden provider (anthropic/openai/google/xai)"
else
    echo "$FB_CHECK"
    fail "fallback_models outside allowlist or forbidden provider (claude-opus-5 regression guard)"
fi

# ── Test 6: All Go-tier agents have a fallback (dual-shape: model key or models[]) ──
echo ""
...
for name, agent in omo.get('agents', {}).items():
    ...
    primary = agent.get('model', '') or (models[0].get('model', '') if models and isinstance(models[0], dict) else (models[0] if models and isinstance(models[0], str) else ''))
    if models and primary.startswith('opencode-go/') and len(models) < 2:
        missing.append(name)
if missing:
    print(', '.join(missing))
else:
...
import re, json5
bad = []
for path in ['$HOME/.config/opencode/opencode.jsonc', '$HOME/.omo/omo.jsonc']:
    omo = json5.loads(open(path).read())
    c = open(path).read()
    for m in re.findall(r'\"model\"\s*:\s*\"([a-zA-Z0-9_./-]+)\"', c):
        if 'deepseek' not in m and 'mimo' not in m and 'minimax' not in m and 'qwen' not in m and 'glm' not in m and 'kimi' not in m and 'grok' not in m and 'gpt' not in m and 'hy3' not in m and 'muse' not in m:
            bad.append(m)
if bad:
    print(', '.join(set(bad)))
else:
...
# ── Test 8: runtime_fallback valid keys (installed schema ground truth) ──
echo ""
echo "--- Premium Model Containment ---"
RF_OK=$(python3 -c "
import json5, os
omo = json5.loads(open(os.path.expanduser('~/.omo/omo.jsonc')).read())
rf = omo['[opencode]'].get('runtime_fallback', {})
expected_keys = {'max_fallback_attempts', 'notify_on_fallback', 'retry_on_errors'}
missing = [k for k in expected_keys if k not in rf]
if missing: print('missing: ' + ','.join(missing))
else: print('ok')
")
if [ "$RF_OK" = "ok" ]; then
    pass "runtime_fallback has expected keys (max_fallback_attempts, notify_on_fallback, retry_on_errors)"
else
    fail "runtime_fallback: $RF_OK"
fi
...
for p in "$HOME/.cache/opencode/packages/oh-my-openagent@latest/node_modules/oh-my-openagent/dist/index.js" \
        "$(npm root -g 2>/dev/null)/oh-my-openagent/dist/index.js"; do
...
omo = set(json5.loads(open('$HOME/.omo/omo.jsonc').read())['[opencode]'].get('agents', {}).keys())
bad = []
for name in oc & omo:
...
# ── Test 11: Category fallback coverage (dual-shape: model key or models[]) ──
echo ""
...
for name, cat in omo.get('categories', {}).items():
    ...
    primary = cat.get('model', '') or (models[0].get('model', '') if models and isinstance(models[0], dict) else (models[0] if models and isinstance(models[0], str) else ''))
    if models and primary.startswith('opencode-go/') and len(models) < 2:
        missing.append(name)
if missing:
    print(', '.join(missing))
...
omo = omo['[opencode]']
issues = []
for k in ['auto_update', 'telemetry', 'model_fallback']:
    if k not in omo:
        issues.append(f'{k}: missing from [opencode] scope')
ws = omo.get('websearch', {})
for k in ['auto_update', 'telemetry', 'model_fallback']:
    if k in ws:
        issues.append(f'{k}: NESTED inside websearch (should be top-level)')
...
")
if [ "$NEW_OK" = "ok" ]; then
    pass "New config features correctly placed (auto_update, telemetry, model_fallback top-level under [opencode], keyword_detector, agent_order, git category)"
else
    fail "$NEW_OK"
fi

# ── Test 13: All 5 experimental flags present ──
echo ""
echo "--- Experimental Flags ---"
EXP_OK=$(python3 -c "
import json5
omo = json5.loads(open('$HOME/.omo/omo.jsonc').read())
exp = omo['[opencode]'].get('experimental', {})
...
# ── Test 17b: OMO unified config exists with completed migrations ──
OMO_OK=$(python3 -c "
import json5, os
h = os.environ['HOME']
path = h + '/.omo/omo.jsonc'
if not os.path.exists(path):
    print('missing file: ' + path)
else:
    omo = json5.loads(open(path).read())
    migs = omo.get('_migrations', [])
    expected = ['2026-07-opencode-config-unification', '2026-08-reasoning-unification']
    missing = [m for m in expected if m not in migs]
    if missing:
...
")
if [ "$OMO_OK" = "ok" ]; then
    pass "omo.jsonc exists at ~/.omo with both migrations applied"
else
    fail "omo.jsonc migrations: $OMO_OK"
fi
...
import json5
omo = json5.loads(open('$HOME/.omo/omo.jsonc').read())
sis = omo['[opencode]'].get('agents', {}).get('sisyphus', {})
...
DCP_OK=$(python3 -c "
import json5, os
dcp = json5.loads(open(os.path.expanduser('~/.config/opencode/dcp.jsonc')).read())
issues = []
if dcp.get('enabled') is not True: issues.append('enabled not True')
tp = dcp.get('turnProtection', {})
if tp.get('enabled') is not True: issues.append('turnProtection.enabled not True')
turns = tp.get('turns')
if turns != 5: issues.append(f'turnProtection.turns={turns} (expected 5)')
if not issues:
    print('ok')
else:
    print('; '.join(issues))
...
import json5
omo = json5.loads(open('$HOME/.omo/omo.jsonc').read())
bs = omo['[opencode]'].get('babysitting', {})
...
import json5
omo = json5.loads(open('$HOME/.omo/omo.jsonc').read())
exit(0 if 'aggressive_truncation' not in omo['[opencode]'] else 1)
" 2>/dev/null; then
...
# ── Test 23: auto-rules .mdc files exist ──
echo ""
echo "--- Auto-Rules ---"
...
# ── Test 26: opencode.jsonc lsp section structure ──
echo ""
echo "--- LSP Config Structure ---"
LSP_OK=$(python3 -c "
import json5, os
h = os.environ['HOME']
cfg = json5.loads(open(h + '/.config/opencode/opencode.jsonc').read())
lsp = cfg.get('lsp', {})
issues = []
if 'typescript' not in lsp: issues.append('typescript entry missing')
EXPECTED = ['basedpyright', 'css', 'html', 'json', 'markdown', 'rust-analyzer', 'typescript']
for srv in EXPECTED:
    if srv not in lsp: issues.append(srv + ' entry missing')
    elif 'command' not in lsp.get(srv, {}): issues.append(srv + '.command missing')
if 'kotlin-ls' in lsp: issues.append('kotlin-ls present (removed 2026-08-03, ADR/014 superseded)')
...
")
if [ "$LSP_OK" = "ok" ]; then
    pass "opencode.jsonc lsp section: 7 servers present, kotlin-ls absent (ADR/014 superseded)"
else
    fail "opencode.jsonc lsp section structure: $LSP_OK"
fi
...
import json5, os, re
h = os.environ['HOME']
omo = json5.loads(open(h + '/.omo/omo.jsonc').read())
omo = omo['[opencode]']
mc = omo.get('background_task', {}).get('modelConcurrency', {})
# Collect all primary models used in categories and agents
used_models = set()
for scope in ['categories', 'agents']:
    for name, obj in omo.get(scope, {}).items():
        models = obj.get('models')
        primary = obj.get('model', '') or (models[0].get('model', '') if models and isinstance(models[0], dict) else (models[0] if models and isinstance(models[0], str) else ''))
        if primary.startswith('opencode-go/'):
...
# ── Test 30: Docs-sync gate (docs must match live config values) ──
echo ""
echo "--- Docs-Sync Gate ---"
DOC_SYNC_OK=$(python3 -c "
import re, os
h = os.environ['HOME']
def slurp(p):
    try: return open(p).read()
    except Exception: return ''
readme = slurp(h + '/.config/opencode/README.md')
cmap = slurp(h + '/.config/opencode/docs/CONFIG_MAP.md')
agents = slurp(h + '/.config/opencode/docs/AGENTS.md')
wf = slurp(h + '/.config/opencode/docs/WORKFLOW.md')
issues = []
# modelConcurrency truth (live omo.jsonc)
if 'muse-spark-1.3-contributor:15' not in cmap: issues.append('CONFIG_MAP modelConcurrency muse-spark-1.3-contributor:15')
if 'muse-spark-1.2-contributor:10' not in cmap: issues.append('CONFIG_MAP modelConcurrency muse-spark-1.2-contributor:15')
...
if issues:
    ...
    print('ok')
")
if [ "$DOC_SYNC_OK" = "ok" ]; then
    pass "Docs reflect live config values (concurrency, disabled_hooks, health checks)"
else
    fail "Docs drifted from config: $DOC_SYNC_OK"
fi
...
cp "$HOME/.omo/omo.jsonc" "$BACKUP_DIR/omo.jsonc" 2>/dev/null
