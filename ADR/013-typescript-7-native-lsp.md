...
OpenCode uses a builtin TypeScript LSP server wired through typescript-language-server. The server is configured via the `resolve-ts-lsp` proxy (bare command name, resolved from `$PATH`) in the `lsp.typescript` section of `opencode.jsonc`.
...
The config lives in the `lsp.typescript` section of `~/.config/opencode/opencode.jsonc` (bare command name `resolve-ts-lsp`). The standalone `lsp.json` file was retired — the live LSP config is read from `opencode.jsonc`.
...
2. `~/.config/opencode/opencode.jsonc` (`lsp.typescript`) contains the `resolve-ts-lsp` entry (bare command name, resolved from `$PATH`).
