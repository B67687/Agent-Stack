...
| `compaction.reserved` | 5000 | Reserve 5k tokens for essential context after compaction |
...
| Setting | Value | Rationale |
|---------|-------|-----------|
| `babysitting.timeout_ms` | **120000** (2 min) | Subagent timeout — longer than 60s default for complex multi-step tasks |
| `dynamic_context_pruning.enabled` | `true` | Auto-prune stale tool outputs — keeps context window fresh |
| `dynamic_context_pruning.turn_protection.turns` | **10** | Default is 3 — we do many turns so prune less aggressively |
| `dynamic_context_pruning.turn_protection.protected_tools` | 7 tools | bash, edit, write, read, lsp_diagnostics, task, glob, grep — never prune critical tool traces |
...
| Setting | Value | Rationale |
|---------|-------|-----------|
| `default_mode` | `ralph` | Self-continuation loop — keeps working until completion criteria met |
| `max_iterations` (ralph) | **20** | Reduced from 200 — stops promptly when task done, no wasted iterations |
| `doom_loop` | `deny` | Safety valve — ralph loop does not bypass the doom loop detector |
...
| `regression-test.sh` | 29 regression tests — config validity, attribution defense, model containment | 2026-07-08 |
...
| Free MiMo | `opencode/mimo-v2.5-free` | Explore, Quick, Unspecified-low, Multimodal-looker, General, Scout | `none` or unset — cheapest model for cheap tasks |
