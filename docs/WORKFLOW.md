...
**What:** A prompt-level instruction injected into Sisyphus, Worker, and Review agents via `prompt_append` in `~/.omo/omo.jsonc` (under the `"[opencode]"` key).
...
**How it works:** The prompt append directs the agent to call `lsp_diagnostics` on each modified file. If the LSP reports errors, the agent must fix them before proceeding. It is not a hard hook (the agent can skip it), but compliance is reinforced by goal-mode verification.
...
**What:** The agent's default operating mode is `goal`, a goal-driven continuation loop with up to 20 iterations. **DORMANT since 2026-08-03** — the goal hook is gated off (`goal.enabled: false`; `"goal"` was removed from `disabled_hooks` 2026-08-04 but the hook stays inert because it is double-gated on `goal.enabled`); re-enabling it makes every user message set the session objective and crashes on >2000-char messages (see INCIDENTS.md Incident 9).

**How it works (if re-enabled):** When configured with `goal: {enabled: true, default_max_iterations: 20}`, the agent automatically continues working toward the stated goal after completing each action. It only stops when it decides the goal is met, hits the iteration limit, or the user intervenes.

**Why it matters:** The ralph loop (the predecessor) was renamed to goal mode to clarify intent: the agent drives toward explicit completion criteria rather than self-perpetuating. The 20-iteration ceiling (down from 200) stops promptly when tasks complete while still allowing multi-phase workflows to converge. **2026-08-03 caveat:** the cap was re-enabled as a guardrail and immediately regressed — the goal hook treats every message as a new objective (hard 2000-char limit) — so it is disabled again. Loop protection now relies on `retry_on_errors` (incl 500) + health-check 5.7 WAL watchdog + keyword-detector disabled.

## 6. Daily Backup Cron

**What:** A cron job at 2 AM daily running `~/.local/share/opencode/scripts/backup-config.sh` (installed 2026-08-04 via crontab — verify with `crontab -l`).

**How it works:** The script copies `opencode.jsonc`, `~/.omo/omo.jsonc`, `tui.json`, and `dcp.jsonc` (if present) to `~/.config/opencode/backups/` with a `YYYYMMDD-` prefix. It prunes backups older than 30 days by file modification time.
...
**What:** `~/.config/opencode/scripts/health-check.sh` - a standalone diagnostic with 9 checks (binary, plugin dir, config readability, disk, omo doctor, schema, WAL, local CI gate wiring, git state).
...
3. Config file readability - opencode.jsonc and ~/.omo/omo.jsonc are present and readable
4. Disk space - home directory usage (informational warning at 90%)
5.5 OMO config load - `omo doctor --verbose` confirms configs parse and load correctly
5.6 Config schema validation - config-schema-check.mjs against installed schema
5.7 SQLite WAL size - WAL >200MB unhealthy (unbounded-writer alert), >100MB degraded (added 2026-08-03)
5.8 Local CI gate wiring - core.hooksPath → scripts/hooks, pre-push hook present + executable (added 2026-08-04)
...
**What:** Eleven `.mdc` rule files in `.opencode/rules/` that auto-activate by path glob pattern, eliminating manual `load_skills` calls for per-language standards.
