...
**How it works:** The `architect` command (`opencode.jsonc` commands section) includes a template that tells the agent: "After each decision, save an ADR to `.omo/adr/<slug>.md` with context, decision, consequences, and alternatives considered." The standard template matches the format already used in `./ADR/`.
...
**What:** The agent's default operating mode is `ralph`, a self-continuation loop with up to 20 iterations.

**How it works:** When set as `default_mode` with `ralph_loop.max_iterations: 20`, the agent automatically continues working after completing each action. It only stops when it explicitly decides it's done, hits the iteration limit, or the user intervenes.

**Why it matters:** The 20-iteration ceiling (down from 200) stops promptly when tasks complete while still allowing multi-phase workflows to converge.
