...
Agent and category models use the `model` key: a single model string per agent/category (e.g. `"model": "opencode-go/mimo-v2.5"`). This is the shape the 4.19.4 runtime's agent-registration code actually reads (`override?.model`); the `models[]` array shape that the `2026-08-reasoning-unification` migration wrote is NOT consumable by the plugin's own registration loop (falls to plugin-default models) — we converted back to `model`-key shape on 2026-08-02. Current routing uses hybrid dispatch: `mimo-v2.5` for general tasks and `muse-spark` for creative/reasoning-heavy work. Example:
